class MyBlock(nn.Module):
    def __init__(self, shape, in_c, out_c, kernel_size=3, stride=1, padding=1, activation=None, normalize=True, dropout_rate=0.1):
        super(MyBlock, self).__init__()
        self.ln = nn.LayerNorm(shape)
        self.conv1 = nn.Conv2d(in_c, out_c, kernel_size, stride, padding)
        self.conv2 = nn.Conv2d(out_c, out_c, kernel_size, stride, padding)
        self.activation = nn.GELU() if activation is None else activation
        self.normalize = normalize
        self.dropout = nn.Dropout(0.1)

    def forward(self, x):
        out = self.ln(x) if self.normalize else x
        out = self.conv1(out)
        out = self.activation(out)
        out = self.dropout(out)
        out = self.conv2(out)
        out = self.activation(out)
        out = self.dropout(out)

        return out


class UNet(nn.Module):
    def __init__(self, in_channels: int=1, 
                 down_channels =[64, 128, 128, 128, 128], 
                 up_channels=[128, 128, 128, 128, 64], 
                 time_emb_dim: int=128,
                 num_classes: int=10) -> None:
        super().__init__()

        # NOTE: You can change the arguments received by the UNet if you want, but keep the num_classes argument

        self.num_classes = num_classes

        # TODO: time embedding layer
        self.time_mlp = nn.Sequential(
            SinusoidalPositionEmbeddings(time_emb_dim),
            nn.Linear(time_emb_dim, time_emb_dim*2),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(time_emb_dim*2, time_emb_dim)
        )


        # TODO: define the embedding layer to compute embeddings for the labels
        self.class_emb = nn.Embedding(num_classes, time_emb_dim)

        # define your network architecture here
        # First half
        self.b1 = nn.Sequential(
            MyBlock((1, 32, 32), 1, 10),
            MyBlock((10, 32, 32), 10, 10),
            MyBlock((10, 32, 32), 10, 10)
        )
        self.down1 = nn.Conv2d(10, 10, 4, 2, 1)

        self.b2 = nn.Sequential(
            MyBlock((10, 16, 16), 10, 20),
            MyBlock((20, 16, 16), 20, 20),
            MyBlock((20, 16, 16), 20, 20)
        )
        self.down2 = nn.Conv2d(20, 20, 4, 2, 1)


        self.b3 = nn.Sequential(
            MyBlock((20, 8, 8), 20, 40),
            MyBlock((40, 8, 8), 40, 40),
            MyBlock((40, 8, 8), 40, 40)
        )
        self.down3 = nn.Conv2d(40, 40, 4, 2, 1)

        # Bottleneck
        self.b_mid = nn.Sequential(
            MyBlock((40, 4, 4), 40, 20),
            MyBlock((20, 4, 4), 20, 20),
            MyBlock((20, 4, 4), 20, 40)
        )

        # Second half
        self.reduce = nn.Conv2d(80, 40, 1)
        self.up1 = nn.ConvTranspose2d(40, 40, 4, 2, 1)

        self.b4 = nn.Sequential(
            MyBlock((40, 8, 8), 40, 40),
            MyBlock((40, 8, 8), 40, 20),
            MyBlock((20, 8, 8), 20, 20)
        )

        self.up2 = nn.ConvTranspose2d(20, 20, 4, 2, 1)

        self.b5 = nn.Sequential(
            MyBlock((40, 16, 16), 40, 20),
            MyBlock((20, 16, 16), 20, 10),
            MyBlock((10, 16, 16), 10, 10)
        )

        self.up3 = nn.ConvTranspose2d(10, 10, 4, 2, 1)

        self.b_out = nn.Sequential(
            MyBlock((20, 32, 32), 20, 10),
            MyBlock((10, 32, 32), 10, 10),
            MyBlock((10, 32, 32), 10, 10, normalize=False)
        )

        self.conv_out = nn.Conv2d(10, 1, 3, 1, 1)

        
    def forward(self, x: torch.Tensor, timestep: torch.Tensor, label: torch.Tensor = None) -> torch.Tensor:
        # TODO: embed time
        t = self.time_mlp(timestep)

        # TODO: handle label embeddings if labels are avaialble
        if label is not None:
            l = self.class_emb(label)
            t = torch.cat((t, l), dim=1)
            
            # Added attention mechanism for label conditioning
            t, _ = self.attention(t.unsqueeze(1), t.unsqueeze(1), t.unsqueeze(1))
            t = t.squeeze(1)

        t = t.mean(dim=1, keepdim=True)
        
        # TODO: compute the output of your network
        n = len(x)
        out1 = self.b1(x + t.reshape(n, -1, 1, 1))  # (N, 10, 32, 32)
        out2 = self.b2(self.down1(out1) + t.reshape(n, -1, 1, 1))  # (N, 20, 16, 16)
        out3 = self.b3(self.down2(out2) + t.reshape(n, -1, 1, 1))  # (N, 40, 8, 8)

        out_mid = self.b_mid(self.down3(out3) + t.reshape(n, -1, 1, 1))  # (N, 40, 3, 3)

        out4_c = torch.cat((out3, self.up1(out_mid)), dim=1)  # (N, 80, 8, 8)
        out4 = self.reduce(out4_c)
        out4 = self.b4(out4 + t.reshape(n, -1, 1, 1))  # (N, 20, 8, 8)

        out5 = torch.cat((out2, self.up2(out4)), dim=1)  # (N, 40, 16, 16)
        out5 = self.b5(out5 + t.reshape(n, -1, 1, 1))  # (N, 10, 16, 16)

        out = torch.cat((out1, self.up3(out5)), dim=1)  # (N, 20, 32, 32)
        out = self.b_out(out + t.reshape(n, -1, 1, 1))  # (N, 1, 32, 32)


        out = self.conv_out(out)

        return out
